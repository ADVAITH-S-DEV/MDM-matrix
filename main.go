...
func main() {
	...
	http.HandleFunc("/enroll", handleEnroll(dbPool))

	http.HandleFunc("/ws", handleWS(dbPool, hub))

	http.HandleFunc("/login", handleLogin(dbPool))

	http.HandleFunc("/devices/", authMiddleware(handleDispatchCommand(dbPool, hub)))
